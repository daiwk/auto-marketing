...
def test_fetch_rejects_non_date_inputs_without_calling_provider(
    monkeypatch, start: object, end: object
) -> None:
...
def test_fetch_rejects_bars_outside_requested_half_open_interval(
    monkeypatch, market_date: str
) -> None:
    frame = raw_frame()
    ...
    monkeypatch.setattr("yfinance.download", lambda *args, **kwargs: frame)
    with pytest.raises(
        DataValidationError, match="SPY.*outside requested interval.*2026-01-02.*2026-01-06"
    ):
        YFinanceSource().fetch("SPY", date(2026, 1, 2), date(2026, 1, 6))
